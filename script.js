(() => {
const LS = { BANK:'bank', EXAMS:'exams', USERS:'users', SESSION:'session', HISTORY:'history' };
...
function renderBank(){
  $('bankList').innerHTML = bank.map(q=>
    `<div class="bank-item">
      <strong>${q.text}</strong> (${q.subject} • ${q.difficulty} • ${q.type})
...
window.editQuestion=id=>{
  ...
  const newSubject=prompt('Edit subject:',q.subject);
  const newDiff=prompt('Edit difficulty:',q.difficulty);
  const newType=prompt('Edit type (MCQ/Multi-select/True/False/Fill/Short):',q.type);
  q.options.forEach((opt,i)=>{
    const newOpt=prompt(`Option ${i+1}:`,opt);
    if(newOpt) q.options[i]=newOpt;
  });
  const newCorrect=prompt('Correct option number (1-4):',q.correct+1);
  if(newText) q.text=newText;
  if(newSubject) q.subject=newSubject;
  if(newDiff) q.difficulty=newDiff;
  if(newType) q.type=newType;
  if(newCorrect && !isNaN(newCorrect) && newCorrect>=1 && newCorrect<=q.options.length) q.correct=+newCorrect-1;
  localStorage.setItem(LS.BANK,JSON.stringify(bank));
  renderBank();
};
...
function renderExamList(){
  $('examList').innerHTML=exams.map(e=>
    `<div>${e.title} (${e.mode}) <button onclick="startExam(${e.id})">Start</button></div>`
  ).join('');
}
...
$('loginBtn').onclick=()=>{
  if(users[$('authUser').value]!==$('authPass').value){ $('authMsg').textContent='Invalid login'; return; }
...
function renderQuestion(){
  ...
  q.options.forEach((o,i)=>{
    ...
    d.onclick=()=>{
      answers[idx]=i;
      if(currentExam.mode==='practice') alert(i===q.correct?'Correct':'Wrong');
      renderQuestion();
    };
...
function submitExam(){
  ...
  currentExam.questions.forEach((q,i)=>{
    ...
    if(ok) score++; else score-=currentExam.neg;
  ...
  $('resultSummary').innerHTML=`Score: ${score}<hr>`+
